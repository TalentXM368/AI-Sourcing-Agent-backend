...
export async function generateEmbeddings(texts: string[]): Promise<number[][]> {
  ...
  if (openaiClient) {
    try {
      ...
      if (isQuota) {
        console.warn('[Embeddings] OpenAI quota exceeded, falling back to local model...')
      } else {
        console.error('[Embeddings] OpenAI failed:', error?.message || error)
      }
    }
  }

  // 2. Try local transformer model (Xenova/all-MiniLM-L6-v2)
  try {
    const { LocalEmbeddingProvider } = await import('../modules/vector-intelligence/providers/local-embedding-provider.js')
    const localProvider = new LocalEmbeddingProvider({
      name: 'local',
      model: process.env.EMBEDDING_MODEL || 'Xenova/all-MiniLM-L6-v2',
      dimensions: EMBEDDING_DIM,
    })
    console.log('[Embeddings] Using local transformer model (first run downloads ~23MB)')
    const vectors = await localProvider.generateEmbeddings(texts)
    console.log(`[Embeddings] Local model produced ${vectors.length} vectors (${vectors[0]?.length || 0}d)`)
    return vectors
  } catch (error) {
    console.error('[Embeddings] Local model failed:', error)
  }

  // 3. Last resort: semantic hash fallback
  console.warn('[Embeddings] All providers failed, using semantic hash fallback')
  return texts.map(text => hashEmbed(text, EMBEDDING_DIM))
}

function hashEmbed(text: string, dimensions: number = 1536): number[] {
  const vector: number[] = new Array(dimensions).fill(0)
  for (let i = 0; i < text.length; i++) {
    const charCode = text.charCodeAt(i)
    vector[i % dimensions] += charCode / 1000
    vector[(i * 7 + 13) % dimensions] += Math.sin(charCode * 0.1) * 0.5
    vector[(i * 13 + 7) % dimensions] += Math.cos(charCode * 0.05) * 0.3
  }
